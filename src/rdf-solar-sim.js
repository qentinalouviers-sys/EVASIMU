...
(function (root) {
      ...
      btns.appendChild(el('a', {
        ...
        onclick: function (ev) {
          ...
          ev.currentTarget.href = 'https://wa.me/' + String(brand.whatsapp).replace(/[^\d]/g, '') +
            '?text=' + encodeURIComponent('Bonjour RDF-SOLAR ! ' + self._leadContext());
        }
      }));
    }
  ...
  Simulator.prototype._renderResults = function () {
    ...
    var cta = el('div', { class: 'rdfsim-card' }, [
      ...
      el('p', { class: 'rdfsim-disclaimer', html: 'Estimation indicative et non contractuelle, calculée à partir de l’ensoleillement moyen de votre région, de l’orientation et de l’inclinaison déclarées. Les arbres plantés dans le simulateur servent à visualiser l’ombrage en 3D mais ne sont pas déduits du calcul : la visite technique (avec relevé drone) affine précisément ces valeurs. ' + (this.catalog.tarifs && this.catalog.tarifs.note ? this.catalog.tarifs.note : '') })
    ]);
    box.appendChild(cta);
  };
